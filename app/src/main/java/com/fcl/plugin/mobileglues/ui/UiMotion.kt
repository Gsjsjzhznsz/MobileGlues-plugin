...
// BandQQ UiMotion 逐字节移植（io.github.gsjsjzhznsz.bandqq.ui.UiMotion）——
// 主题设置页「动画」节的两个滑条（速度/级联延迟）就是驱动这里的。
...
/** 动画速度倍率（0.5 ~ 2.0，越大越快），由 MgGlassTheme 按配置注入 */
val LocalMotionSpeed = staticCompositionLocalOf { 1f }

/** 列表级联入场的逐项间隔（ms，0 ~ 200），由 MgGlassTheme 按配置注入 */
val LocalMotionStagger = staticCompositionLocalOf { 100 }

/**
 * 统一入场动效（速度/延迟均可在「主题 → 动画」调节）：
 * - 时长 = 基础时长 / 动画速度；
 * - 逐项延迟 = index × 级联间隔 / 动画速度；
 * - 效果：延迟后淡入并上移（单次播放），用于各页卡片/列表级联入场。
 */
fun Modifier.listItemReveal(entered: Boolean, index: Int): Modifier = composed {
    ...
    // 级联延迟只作用于前几项：列表很长时不封顶会让末项等数秒才开始出现
